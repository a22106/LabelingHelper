...
class MainWindow(QMainWindow):
    ...
    def refreshFileName(self):
        ...
        if extract_path:
            ...
            clip_path_list = [clipPath for clipPath in clip_path_list if 'Clip' in clipPath and os.path.isdir(clipPath)]
    ...
    def backup(self):
        if self.inputPath is None:
            ...
            for idx, backup_file in enumerate(backup_file_list):
                self.printText(f'{idx+1}/{backup_file_num} {backup_file} 백업 완료')
            ...
            if obj_id:
                ...
                if frame_id_exist:
                    if len(frame_id_exist) >= 100:
                        ...
                        self.printText(f'객체 id {obj_id}가 프레임 {frame_id_exist[0]} ~ {frame_id_exist[-1]} 사이 {unique_frame_num}개의 프레임에 {len(frame_id_exist)}개 있습니다.')
                    else:
                        self.printText(f'객체 id {obj_id}가 프레임 {frame_id_exist}에 {len(frame_id_exist)}개 있습니다.')
                        
                    self.printText(f'프레임 {frame_id_exist[0]} 내 객체 id {obj_id}의 정보:')
    ...
    def changeDimension(self):
        if self.inputPath is None or os.path.isdir(self.resultPath) is False:
            ...
            if obj_id:
                box, frames = self.editLabel.changeDim(obj_id, width, height, length)
                self.printText(f'ID: {obj_id}의 크기를 (width:{box[0]:.03f}, height:{box[1]:.03f}, length:{box[2]:.03f})로 변경 완료')
                self.printText(f'객체 변경 프레임: \n{frames}')
                print("----------------------------------------------------")
                return True
            else:
    ...
    def changeAngle(self):
        if self.inputPath is None or os.path.isdir(self.resultPath) is False:
            ...
            if obj_id and angle>=0:
                frames, former_angle = self.editLabel.changeAngle(obj_id, angle)
                self.printText('객체 id({})의 각도 변경 완료. {}˚ -> {}˚'.format(obj_id, former_angle, angle))
    ...
    def changeAngle180(self):
        if self.inputPath is None or os.path.isdir(self.resultPath) is False:
            ...
            obj_id = self.ui.spinBoxfromIdchange_3.value()
            if obj_id:
                frames, cur_angle_degree, changed_angle_degree = self.editLabel.changeAngle180(obj_id)
                self.printText('객체 id({})의 각도 변경 완료. {:.3f} -> {:.3f}'.format(obj_id, cur_angle_degree, changed_angle_degree))
                self.printText('객체 변경 프레임: \n{}'.format(frames))
                return True
            else:
                self.printText('객체 아이디 혹은 각도를 다시 확인해주세요.')
    ...
    def changeCategory(self)->bool:
        ...
        if obj_id and obj_category:
            ...
            if obj_category < 8:
                ...
                obj_type = 1
            frames = self.editLabel.changeCategory(obj_id, obj_type, _CATEGORY[obj_category])
            self.printText('객체 {}의 카테고리를 {}({})로 변경 완료'.format(obj_id, _OBJTYPE[obj_type], _CATEGORY[obj_category]))
        ...
        if self.inputPath is None or os.path.isdir(self.resultPath) is False:
            ...
            if self.editLabel.resultNum > 100:
                ...
                if obj_id and pick_frame and start_frame and end_frame and ok:
                    frame_copy, frame_paste = self.editLabel.copyObject(obj_id, pick_frame, start_frame, end_frame, is_change)
                    self.printText('복사 객체 id: {} 프레임: {} ~ {}'.format(obj_id, start_frame, end_frame))
    ...
    def checkresult(self):
        ...
        if os.path.isdir(self.editLabel.resultPath):
            if len(self.editLabel.resultList) > 0:
                ...
                obj_num_first = len(self.editLabel[0]['annotation']) # 첫 프레임 객체 수
                ...
                if self.editLabel.resultNum > 1:
                    frame_num_last = self.editLabel[-1]['frame_no'] # 마지막 프레임 번호
                    obj_num_last = len(self.editLabel[-1]['annotation']) # 마지막 프레임 객체 수
    ...
    def mainWindow(self):
        ...
        self.ui.pushButton_11.clicked.connect(self.check_object_id)           # 객체 아이디 체크
        ...
        self.ui.pushButton_8.clicked.connect(self.copy_object)               # 객체 복사
